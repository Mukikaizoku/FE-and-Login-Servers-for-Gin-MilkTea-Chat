...
namespace ChatServer
{
    class SessionManager
    {
        ...
        public static void InitServerProtocol(Protocol protocol)
        {
        ...
        public Session MakeNewWebSession(WebToProtocolTranslator webSocket)
        {
            Session newSession = null;
            lock (connectedSessions)
            {
            ...
            newSession.Init(webSocket, backEndSession);
        ...
        private void RemoveSession(Session session)
        {
            lock(connectedSessions)
            {
                if(session != null && connectedSessions.ContainsKey(session.sessionId))
                {
                    ...
                    Console.WriteLine("[**EXIT*]" + new string(session.id) +"Client " + session.sessionId + " (" + session.ip + ") has exited or timed out");
